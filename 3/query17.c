...
int duplicateCourseDifferentMarks(int load, char courses[][30], int grades[]){
	int courseRepeat[10] = {0};
	int uniqueDuplicates = 0;

	for(int j = 0; j < load - 1; j++){
		for(int k = j + 1; k < load; k++){
			if((strcmp(courses[j], courses[k]) == 0) && (grades[j] != grades[k]))
				return 1;
		}
	}
	return 0;
...
int main(int argc, char *argv[]) {
	...
	if (db){							
		...
		for (int i = 0; i < size; ++i){ // process all the student records in database
			...
			if(duplicateCourseDifferentMarks(s.load, s.courses, s.grades)){
